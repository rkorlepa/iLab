...
if __name__ == "__main__":
    try:
        ...
        switches = []
        switches = db.select('', "start_time is NULL or start_time<='"+str(datetime.now())+"'", '*')
        for switch in switches:
            start_end = util.get_start_end_time(switch)
            db.update_start_end_time(switch['switch_name'], start_end['start'], start_end['end'])
        
        switches = []
        switches = db.select('', "start_time<='"+str(datetime.now())+"'", '*')
        for switch in switches:
            p = Process(target=pFunc.powerOn, args=(switch, db))
            p.start()
            processes.append(p)
        for pr in processes:
            pr.join()
